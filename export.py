...
def parse_opt(known=False):
    ...
    parser.add_argument('--output_path',type=str,default='experiment_models')
...
def main(opt):
    ...
    model_no_detect = torch.nn.Sequential(*[model.model[i] for i in range(15)])

    path = f'{opt.output_path}/{opt.weights.split("/")[-1].replace("pt","onnx")}'
    ...
    detect_module = model.model[15]
    torch.save(detect_module.state_dict(), f'{opt.output_path}/detect_module.pt')
